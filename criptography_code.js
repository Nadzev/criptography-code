    ...
    ftp://ftp.isi.edu/in-notes/rfc3713.txt
    
    
    !! 当スクリプトは本家で公開されている JAVA のソースを元に
    	JavaScript にて記述してあります。
    ...
    * 暗号化
    cipherText = camellia.encrypt( plainText,  key [, mode [, iv [, paddingChar]]] );

    * 復号化
    plainText = camellia.decrypt( cipherText, key [, mode [, paddingChar]] );
    ...
    * 暗号･復号共に連想配列で渡すことも出来る
    ...
    cipherText = camellia.encrypt( myHash );
    ...
    * 拙作の URL エンコード/デコード関数を使う ( 同スクリプトに有り )
    ...
    iv          ->  16 byte (ecb モードでは不要)

    paddingChar ->  平文を16の倍数にするために使用するキャラクタ。
    ...
    * バイナリモード
    ...
    ex)
    camellia.binMode = 1;
    cipherData       = camellia.encrypt( 略.... );
    
    上記の様に暗号・復号前に camellia.binMode に真となる値を代入することで
    ...
    camellia = new function()
    {
        ...
        var argDKeyz    = ["data", "key", "mode", "pchar"];
        this.binMode    = 0;
        ...
        var round = function(_blks)
        {
            var dz = toBigEndian32(_blks);
            var j, i, k;
            ...
            for (k=4, i=0; i<3; i++)
            {
                ...
                if (i < 2)
                {
                    dz[1] ^= rotL(dz[0] & keyz[k], 1);
                    dz[0] ^= dz[1] | keyz[k+ 1];
                    dz[2] ^= dz[3] | keyz[k+ 3];
                    dz[3] ^= rotL(dz[2] & keyz[k+ 2], 1);
                    k += 4;
                }
            }
        ...
        var feistelSbox = function(_x, _t)
        {
            return !_t?
                  SBOX4[_x & 0xff] ^ SBOX3[(_x >>> 8 ) & 0xff]
                ^ SBOX2[(_x >>> 16) & 0xff] ^ SBOX1[(_x >>> 24) & 0xff]:
                
                  SBOX1[_x & 0xff] ^ SBOX4[(_x >>> 8 ) & 0xff]
                ^ SBOX3[(_x >>> 16) & 0xff] ^ SBOX2[(_x >>> 24) & 0xff];
        }
    
        var feistelEnc = function(_ki, _dz)
        {
            var D, U;
            U = feistelSbox(_dz[0] ^ keyz[_ki]);
            D = feistelSbox(_dz[1] ^ keyz[1+ _ki], 1);
            _dz[2] ^= D ^ U ;
            _dz[3] ^= D ^ U ^ rotR(U, 8); 
            
            U = feistelSbox(_dz[2] ^ keyz[2+ _ki]);
            D = feistelSbox(_dz[3] ^ keyz[3+ _ki], 1);
            _dz[0] ^= D ^ U  ;
            _dz[1] ^= D ^ U ^ rotR(U, 8) ;
        }
    
        var feistelDec = function(_ki, _dz)
        {
            var D, U;
            U = feistelSbox(_dz[0] ^ keyz[_ki]);
            D = feistelSbox(_dz[1] ^ keyz[1+ _ki], 1);
            ...
            U = feistelSbox(_dz[2] ^ keyz[_ki- 2]);
            D = feistelSbox(_dz[3] ^ keyz[_ki- 1], 1);
            _dz[0] ^= D ^ U  ;
            _dz[1] ^= D ^ U ^ rotR(U, 8) ;
        }
        
        var feistel = function (_x, _xi, _k, _ki, _offset)
        {
            var D, U;
            U = feistelSbox(_x[_xi] ^ _k[_ki]);
            D = feistelSbox(_x[1+ _xi] ^ _k[1+ _ki], 1);
            _x[2+ _xi] ^= D ^ U ;
            _x[3+ _xi] ^= D ^ U ^ rotR(U, 8); 
            
            U = feistelSbox(_x[2+ _xi] ^ _k[_offset+ _ki]);
            D = feistelSbox(_x[3+ _xi] ^ _k[_offset+1 + _ki], 1);
            _x[   _xi] ^= D ^ U;
            _x[1+ _xi] ^= D ^ U ^ rotR(U, 8);
        }
        
        var keyUpdate = function(_key)
        {
            var kLen = _key.length;
            var padF = function(_l, _n) 
            {
                var tmp = [];
                var i = _l-_n;
                while (i--) tmp[i] = 0;
            ...
            if (kLen <= 16)
                _key = _key.concat(padF(16, kLen));
            else if (kLen <= 24)
                _key = _key.concat(padF(24, kLen));
            else if (kLen < 32) 
                _key = _key.concat(padF(32, kLen));
            else if (kLen > 32) 
                _key.length = 32;
    
            return ekeygen(toBigEndian32(_key));
        }
    
        var ekeygen = function(_rawKey)
        {
            var t = _rawKey.slice(0, 4);
            var i, n;
            for (i=4; i<8; i++) t[i] = 0;
            ...
            var kt = t;
            for (n=4, i=8; i<16; i++, n++) kt[i] = t[n];
            
            for (i = 4; i < 26; i += 2 ){
                rotBlock(t , KIDX1[i], KSFT1[i], kt, i* 2);
                rotBlock(t , KIDX1[i+ 1], KSFT1[i+ 1], kt, i* 2+ 2);
            }
            return kt;
        }
    
        var rotBlock = function(_x, _xi, _n, _y, _yi)
        {
            var r   = (_n & 0x1f);
            var idx = (_n >>> 5);
            var idx1= (idx+ 1) & 0x03;
            var idx2= (idx1+ 1) & 0x03;
            
            _y[_yi] = (_x[idx+ _xi] << r) | (_x[idx1+ _xi] >>> (32- r));
            _y[_yi+ 1] = (_x[idx1+ _xi] << r) | (_x[idx2+ _xi] >>> (32- r));
        }
    
        var xorBlock = function(_x , _xi , _y, _yi, _z, _zi)
        {
            _z[   _zi] = _x[   _xi] ^ _y[   _yi];
            _z[1+ _zi] = _x[1+ _xi] ^ _y[1+ _yi];
            _z[2+ _zi] = _x[2+ _xi] ^ _y[2+ _yi];
            _z[3+ _zi] = _x[3+ _xi] ^ _y[3+ _yi];
        }
        
        var rotR = function(_x, _s){
             return (_x >>> _s) | ((_x << (32 - _s)) & 0xffffffff);
        }
         
        var rotL = function(_x, _s){
            return ((_x << _s) & 0xffffffff) | (_x >>> (32 - _s)) ;
        }
        ...
        var SIGMA1 = [ 0xa09e667f, 0x3bcc908b, 0xb67ae858, 0x4caa73b2 ];
        var SIGMA2 = [ 0xc6ef372f, 0xe94f82be, 0x54ff53a5, 0xf1d36f1c ];
        var SIGMA3 = [ 0x10e527fa, 0xde682d1d, 0xb05688c2, 0xb3e6c1fd ];
...
plainText = camellia.decrypt( cipherText, "myKey", "cbc","\x05");
